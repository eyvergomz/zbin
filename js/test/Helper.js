...
describe('Helper', function () {
    describe('secondsToHuman', function () {
        jsc.property('returns an array with a number and a word', 'integer', function (number) {
            var result = $.Zbin.Helper.secondsToHuman(number);
        ...
        jsc.property('returns months on the first array position', 'integer 5184000 9007199254740991', function (number) {
            return $.Zbin.Helper.secondsToHuman(number)[0] === Math.floor(number / (60 * 60 * 24 * 30));
        });
        jsc.property('returns months on the second array position', 'integer 5184000 9007199254740991', function (number) {
            return $.Zbin.Helper.secondsToHuman(number)[1] === 'month';
        });
    });
        ...
        jsc.property(
            ...
            function (ids, contents) {
                ...
                ids.forEach(function(item, i) {
                    html += '<div id="' + item.join('') + '">' + $.Zbin.Helper.htmlEntities(contents[i] || contents[0]) + '</div>';
                });
                ...
                /*
                ids.forEach(function(item, i) {
                    $.Zbin.Helper.selectText(item.join(''));
    ...
    describe('sprintf', function () {
        jsc.property(
            ...
            function (prefix, params, postfix) {
                ...
                return result === $.Zbin.Helper.sprintf.apply(this, params);
            }
        );
        jsc.property(
...
/* TODO test fails since jsDOM version 17 - document.cookie remains empty
        jsc.property(
            ...
            function (labels, values) {
                ...
                const result = $.Zbin.Helper.getCookie(selectedKey);
                $.Zbin.Helper.reset();
                clean();
    ...
    describe('htmlEntities', function () {
        ...
        jsc.property(
            ...
            'string',
            function (string) {
                var result = $.Zbin.Helper.htmlEntities(string);
    ...
    describe('formatBytes', function () {
        jsc.property('returns 0 B for 0 bytes', function () {
            return $.Zbin.Helper.formatBytes(0) === '0 B';
        });

        jsc.property('formats bytes < 1000 as B', function () {
            return $.Zbin.Helper.formatBytes(500) === '500 B';
        });

        jsc.property('formats kilobytes correctly', function () {
            return $.Zbin.Helper.formatBytes(1500) === '1.5 kB';
        });

        jsc.property('formats megabytes correctly', function () {
            return $.Zbin.Helper.formatBytes(2 * 1000 * 1000) === '2 MB';
        });

        jsc.property('formats gigabytes correctly', function () {
            return $.Zbin.Helper.formatBytes(3.45 * 1000 * 1000 * 1000) === '3.45 GB';
        });

        jsc.property('formats terabytes correctly', function () {
            return $.Zbin.Helper.formatBytes(1.75 * 1000 ** 4) === '1.75 TB';
        });

        jsc.property('formats petabytes correctly', function () {
            return $.Zbin.Helper.formatBytes(1.5 * 1000 ** 5) === '1.5 PB';
        });

        jsc.property('formats exabytes correctly', function () {
            return $.Zbin.Helper.formatBytes(1.2345 * 1000 ** 6).startsWith('1.23 EB');
        });

        jsc.property('formats yottabytes correctly', function () {
            return $.Zbin.Helper.formatBytes(1.23 * 1000 ** 8).startsWith('1.23 YB');
        });

        jsc.property('rounds to two decimal places', function () {
            return $.Zbin.Helper.formatBytes(1234567) === '1.23 MB';
        });
    });
    ...
    describe('isBootstrap5', function () {
        jsc.property('Bootstrap 5 has been detected', function () {
            global.bootstrap = {};
            return $.Zbin.Helper.isBootstrap5() === true;
        });

        jsc.property('Bootstrap 5 has not been detected', function () {
            delete global.bootstrap;
            return $.Zbin.Helper.isBootstrap5() === false;
        });
    });
});
