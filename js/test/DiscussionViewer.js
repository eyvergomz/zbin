    ...
    describe('handleNotification, prepareNewDiscussion, addComment, finishDiscussion, getReplyMessage, getReplyNickname, getReplyCommentId & highlightComment', function () {
        ...
        jsc.property(
            ...
            function (comments, commentKey, fadeOut, nickname, message, alertType, alert) {
                ...
                $.Zbin.Model.init();
                $.Zbin.DiscussionViewer.init();
                results.push(
                    !$('#discussion').hasClass('hidden')
                );
                $.Zbin.DiscussionViewer.prepareNewDiscussion();
                results.push(
                    $('#discussion').hasClass('hidden')
                );
                comments.forEach(function (comment) {
                    ...
                    $.Zbin.DiscussionViewer.addComment($.Zbin.Helper.CommentFactory(comment), comment.data, comment.meta.nickname);
                });
                results.push(
                    $('#discussion').hasClass('hidden')
                );
                $.Zbin.DiscussionViewer.finishDiscussion();
                results.push(
                    !$('#discussion').hasClass('hidden') &&
                ...
                if (comments.length > 0) {
                    ...
                    $.Zbin.DiscussionViewer.highlightComment(comments[commentKey].id, fadeOut);
                    results.push(
                ...
                $.Zbin.DiscussionViewer.getReplyCommentId();
                results.push(
                    $.Zbin.DiscussionViewer.getReplyNickname() === $('#reply #nickname').val() &&
                    $.Zbin.DiscussionViewer.getReplyMessage() === $('#reply #replymessage').val()
                );
                var notificationResult = $.Zbin.DiscussionViewer.handleNotification(alertType === 'other' ? alert : alertType);
