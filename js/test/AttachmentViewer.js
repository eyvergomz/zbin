    ...
    describe('setAttachment, showAttachment, removeAttachment, hideAttachment, hideAttachmentPreview, hasAttachment, getAttachment & moveAttachmentTo', function () {
        ...
        jsc.property(
            ...
            function (mimeType, rawdata, filename, prefix, postfix) {
                ...
                $.Zbin.AttachmentViewer.init();
                $.Zbin.Model.init();
                results.push(
                    !$.Zbin.AttachmentViewer.hasAttachment() &&
                ...
                if (filename.length) {
                    $.Zbin.AttachmentViewer.setAttachment(data, filename);
                } else {
                    $.Zbin.AttachmentViewer.setAttachment(data);
                }
                // // beyond this point we will get the blob URL instead of the data
                data = window.URL.createObjectURL(data);
                const attachment = $.Zbin.AttachmentViewer.getAttachments();
                results.push(
                    $.Zbin.AttachmentViewer.hasAttachment() &&
                    $('#attachment').hasClass('hidden') &&
                    $('#attachment').children().length > 0 &&
                ...
                $.Zbin.AttachmentViewer.showAttachment();
                results.push(
                    !$('#attachment').hasClass('hidden') &&
                    $('#attachment').children().length > 0 &&
                    (previewSupported ? !$('#attachmentPreview').hasClass('hidden') : $('#attachmentPreview').hasClass('hidden'))
                );
                $.Zbin.AttachmentViewer.hideAttachment();
                results.push(
                    $('#attachment').hasClass('hidden') &&
                    (previewSupported ? !$('#attachmentPreview').hasClass('hidden') : $('#attachmentPreview').hasClass('hidden'))
                );
                if (previewSupported) {
                    $.Zbin.AttachmentViewer.hideAttachmentPreview();
                ...
                let element = $('<div>');
                $.Zbin.AttachmentViewer.moveAttachmentTo(element, attachment[0], prefix + '%s' + postfix);
                ...
                if (prefix.indexOf('<a') === -1 && postfix.indexOf('<a') === -1) {
                    ...
                    result = DOMPurify.sanitize(
                        prefix + $.Zbin.Helper.htmlEntities(filename) + postfix, {
                ...
                $.Zbin.AttachmentViewer.removeAttachment();
                results.push(
                    $('#attachment').hasClass('hidden') &&
                    $('#attachment').children().length === 0 &&
                    $('#attachmentPreview').hasClass('hidden')
        ...
        it(
            ...
            function() {
                ...
                for (const filename of maliciousFileNames) {
                    $.Zbin.AttachmentViewer.setAttachment('data:;base64,', filename);
