/**
 * Modulo Principal del Frontend de Zbin
 *
 * Este archivo contiene toda la logica del lado del cliente para Zbin,
 * un pastebin de conocimiento-cero (zero-knowledge).
 *
 * Arquitectura del frontend (modulos principales):
 *
 * - CryptoData / Paste / Comment: Clases para manejar datos cifrados y sus formatos
 * - Helper: Funciones utilitarias (formateo de fechas, tamanos, URLs, cookies, etc.)
 * - I18n: Internacionalizacion - traducciones del lado del cliente
 * - CryptTool: Cifrado/descifrado usando Web Crypto API (AES-256-GCM)
 * - Model: Modelo de datos del frontend (cache de pastes, IDs, claves)
 * - UiHelper: Utilidades de interfaz (scroll, visibilidad, historial)
 * - Alert: Sistema de notificaciones (errores, advertencias, estados)
 * - PasteStatus: Manejo del estado del paste (URL, tiempo restante, acortador)
 * - Prompt: Dialogos modales (contrasena, confirmacion de carga)
 * - Editor: Editor de texto con soporte de tabulaciones y vista previa
 * - PasteViewer: Visualizador de pastes (markdown, codigo, texto plano)
 * - AttachmentViewer: Manejo de archivos adjuntos (drag & drop, previsualizacion)
 * - DiscussionViewer: Sistema de comentarios/discusion en pastes
 * - TopNav: Barra de navegacion superior (botones, opciones, formato)
 * - ServerInteraction: Comunicacion con el servidor (envio/recepcion de pastes)
 * - PasteEncrypter: Cifrado de pastes antes de enviarlos al servidor
 * - PasteDecrypter: Descifrado de pastes recibidos del servidor
 * - CopyToClipboard: Funcionalidad de copiar al portapapeles
 * - PasswordPeek: Mostrar/ocultar contrasena
 * - Controller: Controlador principal que orquesta todos los modulos
 *
 * 
 * @name      Zbin
 * @namespace
...
/**
 * Plugin jQuery para detectar cuando se arrastra un archivo sobre un elemento.
 * Emite eventos personalizados 'draghoverstart' y 'draghoverend'.
 * Se usa para mostrar la zona de arrastre de archivos adjuntos.
 */
...
// Inicio de la aplicacion: se ejecuta cuando el DOM esta completamente cargado
jQuery(document).ready(function() {
    'use strict';
    // Inicializar el controlador principal del frontend
    $.Zbin.Controller.init();
});

/**
 * Modulo principal de Zbin - Contiene todos los sub-modulos del frontend.
 * Se estructura como un modulo auto-ejecutable (IIFE) que expone la API publica
 * a traves del objeto jQuery.Zbin (accesible como $.Zbin).
 */
jQuery.Zbin = (function($) {
    'use strict';

    /**
     * Interfaz con la libreria zlib para compresion/descompresion
     *
     * @private
     */
    let z;
    ...
    /**
     * Configuracion de DOMPurify para contenido HTML
     * Define que URIs son permitidas y habilita el perfil HTML
     *
     * @private
     */
    ...
    /**
     * Configuracion estricta de DOMPurify para HTML
     * Solo permite un subconjunto muy limitado de etiquetas (a, i, span, kbd)
     * para traducciones que contienen HTML seguro.
     *
     * NOTA: No incluir USE_PROFILES aqui porque toma precedencia sobre ALLOWED_TAGS.
     *
     * @private
     */
   const purifyHtmlConfigStrictSubset = {
    ...
    /**
     * Configuracion de DOMPurify para contenido SVG (iconos de comentarios)
     *
     * @private
     */
    ...
    /**
     * Prefijo de fragmento URL que requiere confirmacion de carga
     * Si el hash de la URL empieza con '#-', se pide confirmacion al usuario
     *
     * @private
     */
     ...
     * Clase CryptoData
     *
     * Clase base que agrupa funciones auxiliares comunes para datos cifrados.
     * Es la clase padre de Paste y Comment.
     *
     * @name CryptoData
     * @class
     */
    ...
    /**
     * Clase Paste (Documento)
     *
     * Representa un documento cifrado con sus metadatos.
     * Proporciona acceso al formato, tiempo de vida, y flags
     * de configuracion (burn-after-reading, discusion).
     *
     * @name Paste
     * @class
     */
     ...
     *
     * Representa un comentario cifrado en una discusion.
     * Proporciona acceso a la fecha de creacion y el icono del autor.
     *
     * @name Comment
     * @class
     */
    ...
    /**
     * Modulo de funciones auxiliares estaticas
     *
     * Contiene utilidades de uso general: formateo de tiempo y tamano,
     * manipulacion de URLs, cookies, codificacion HTML, etc.
     *
     * @name Helper
     * @class
     */
    ...
    /**
     * Modulo de Internacionalizacion (i18n)
     *
     * Maneja las traducciones del lado del cliente.
     * Carga archivos JSON de traduccion segun el idioma del navegador o la cookie.
     * Soporta plurales con reglas especificas por idioma.
     *
     * @name I18n
     * @class
     */
    const I18n = (function () {
        ...
        me.loadTranslations = function()
        {
            ...
            const cacheBreaker = document.querySelector('script[src^="js/zbin.js"]').getAttribute('src').split('.js')[1] || '';
    ...
    /**
     * Modulo de Cifrado/Descifrado
     *
     * Maneja toda la criptografia del lado del cliente usando la Web Crypto API.
     * Algoritmo: AES-256-GCM con PBKDF2 para derivacion de claves.
     * Soporta compresion zlib y codificacion base58 para las claves en la URL.
     *
     * El cifrado es "zero-knowledge": el servidor nunca ve los datos sin cifrar
     * ni la clave de cifrado (que esta en el fragmento # de la URL).
     *
     * @name CryptTool
     * @class
     */
    ...
    /**
     * Modelo de datos del frontend (patron MVC)
     *
     * Almacena en cache los datos del paste, el ID, la clave simetrica
     * y las plantillas HTML clonables. Centraliza el acceso a los datos.
     *
     * @name   Model
     * @class
     */
     ...
     *
     * Funciones auxiliares para la UI: scroll suave, deteccion de visibilidad
     * de elementos, manejo del historial del navegador y recarga de pagina.
     *
     * @name   UiHelper
     * @class
     */
    const UiHelper = (function () {
         ...
         * reload the page
         *
         * This takes the user to the Zbin homepage.
         *
     ...
     *
     * Maneja la visualizacion de mensajes al usuario: errores, advertencias,
     * estados de carga, mensajes de exito y tiempo restante de expiracion.
     *
     * @name   Alert
     * @class
     */
    ...
    /**
     * Manejo del Estado del Paste
     *
     * Gestiona la URL del paste creado, el enlace de eliminacion,
     * el acortamiento automatico de URLs y el tiempo restante de expiracion.
     *
     * @name   PasteStatus
     * @class
     */
    ...
    /**
     * Dialogos de Solicitud al Usuario
     *
     * Maneja el modal de contrasena y la confirmacion de carga de paste.
     * La contrasena se usa como factor adicional en la derivacion de la clave de cifrado.
     *
     * @name Prompt
     * @class
     */
     ...
     * Editor de Texto
     *
     * Gestiona el area de texto para escribir pastes, incluyendo soporte
     * para tabulaciones y la pestana de vista previa.
     * La vista previa en si es manejada por PasteViewer.
     *
     * @name   Editor
     * @class
     */
    ...
    /**
     * Visualizador de Pastes
     *
     * Analiza y muestra el contenido descifrado del paste.
     * Soporta tres formatos: texto plano, resaltado de sintaxis (prettify)
     * y Markdown (showdown). Convierte URLs en enlaces clickeables.
     *
     * @name   PasteViewer
     * @class
     */
     ...
     *
     * Gestiona la subida, visualizacion y descarga de archivos adjuntos.
     * Soporta drag & drop, pegado desde portapapeles, previsualizacion
     * de imagenes/audio/video, y conversion a Blob URLs para descarga.
     *
    ...
    /**
     * Visor de Discusiones
     *
     * Muestra el hilo de comentarios de un paste y gestiona las respuestas.
     * Los comentarios se organizan en un arbol de respuestas y se pueden
     * resaltar mediante el hash de la URL (#comment_xxx).
     *
     ...
     *
     * Gestiona todos los controles de la barra superior: botones de accion
     * (nuevo, clonar, eliminar, descargar), selectores (formato, expiracion,
     * idioma, plantilla), opciones (discusion, burn-after-reading, contrasena)
     * y funcionalidades como codigo QR, email y texto sin formato.
     *
     * @name   TopNav
     * @param  {object} window
     ...
     *
     * Gestiona las peticiones AJAX al servidor. Maneja transparentemente
     * el cifrado/descifrado de datos antes de enviar/recibir.
     * Implementa la logica de reintentos y manejo de errores HTTP.
     *
     ...
     * Cifrador de Pastes
     *
     * Se encarga de cifrar el documento y enviarlo al servidor.
     * El cifrado real lo hace ServerInteraction de forma transparente.
     * Gestiona el flujo completo: preparar datos, cifrar, enviar,
     * y mostrar la URL resultante al usuario.
     *
     * @name PasteEncrypter
     * @class
     */
    ...
    /**
     * Descifrador de Pastes
     *
     * Se encarga de descifrar los datos recibidos del servidor y pasarlos
     * a la vista para su presentacion. Maneja el flujo completo de descifrado:
     * solicitar contrasena si es necesario, descifrar, y mostrar el contenido.
     *
     * @name PasteDecrypter
     * @class
     */
     ...
     *
     * Gestiona la funcionalidad de copiar texto y enlaces al portapapeles.
     * Soporta boton de copia, atajo de teclado (Ctrl+C) y muestra
     * iconos de confirmacion al copiar exitosamente.
     *
     ...
     *
     * Permite al usuario ver temporalmente la contrasena que esta escribiendo
     ...
     *
     * Orquesta todos los modulos del frontend. Se encarga de:
     * - Inicializar todos los sub-modulos al cargar la pagina
     * - Coordinar la creacion, lectura y clonacion de pastes
     * - Gestionar las transiciones de estado de la interfaz
     * - Inicializar la libreria zlib para compresion
     *
     * @name   Controller
     * @param  {object} window
     * @param  {object} document
